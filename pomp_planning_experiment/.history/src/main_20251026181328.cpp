    ...
    while (true)
    {
...

        {
            ...
            if (valid_pomp_jps_path)
            {
                ...
                float path_length = get_distance(path_grid);
                ...
                std::cout << "Pomp JPS planning successful." << std::endl;
            }
            else
            {
        ...
        grid_map->insertPcd(env->points_array);
        {
            ...
            if (valid_grid_jps_path)
            {
                ...
                float path_length = get_distance(path_grid);
                grid_path_length = grid_path_length + path_length;
        ...
        if (current_frame == nullptr|| frame_idx==1999)
        {

            std::cout << "frame amount: " << frame_idx << std::endl;
            std::cout << "pomp success: " << pomp_success << std::endl;
            std::cout << "grid success: " << grid_success << std::endl;
            std::cout << "pomp average path distance: " << pomp_path_length / (frame_idx+1) << std::endl;
            std::cout << "grid average path distance: " << grid_path_length / (frame_idx+1) << std::endl;
