...
public:
    ...
    void move_step(const std::array<double, 3> &step, int dir)
    {
        ...
        geom_pos.row(idx) = geom_pos.row(idx) + stp.transpose();
    }
};
...
public:
    ...
    inline double urand(std::mt19937 &gen, double a, double b)
    {
        std::uniform_real_distribution<double> dist(a, b);
        return dist(gen);
    }
    void move_one_step(int cube_idx)
    {
        ...
        for (int i = 0; i < cloud_boxes.size(); i++)
        {
            ...
            double speed = urand(gen, boxes_param.speed_min, boxes_param.speed_max);

            cube.move_step(boxes_param.speed, dir[i]);
        }
    }
};
