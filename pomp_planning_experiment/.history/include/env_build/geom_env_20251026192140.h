...
bool not_in_start_goal_area(double x, double y, double z, Eigen::Vector3d start_,
                            Eigen::Vector3d goal_, double margin = 5.0)

{
    double dist_start = std::sqrt(std::pow(x - start_[0], 2) + std::pow(y - start_[1], 2) + std::pow(z - start_[2], 2));
    double dist_goal = std::sqrt(std::pow(x - goal_[0], 2) + std::pow(y - goal_[1], 2) + std::pow(z - goal_[2], 2));
    if (dist_start < margin || dist_goal < margin)
        return false;
    else
        return true;
}
...
void build_env_pointcloud(std::unique_ptr<animation_frames> &cloud_frames, int seed_ = 42)
{

    std::unique_ptr<geom_env> cubes_ = std::make_unique<geom_env>();
    // 3) optional knobs
    double surface_density = 10.0; // points per m^2 on surfaces
    int seed = seed_;
    ...
    //     double x = urand(gen, s, Lx - s) - map_size[0] / 2.f;
    //     double y = urand(gen, s, Ly - s) - map_size[1] / 2.f;
    //     double z = urand(gen, s, Lz - s) - map_size[2] / 2.f;
    //     sample_boxes_surface(cubes_, gen, x, y, z, s, surface_density);
    // }
    int i = 0;
    while (i < boxes_param.num_boxes)
    {
        double s = rx_boxes_s(gen);
        // keep inside AABB with margin r
        ...
        double y = urand(gen, s, Ly - s) - map_size[1] / 2.f;
        double z = urand(gen, s, Lz - s) - map_size[2] / 2.f;
        if (not_in_start_goal_area(x, y, z, {-20, -20, -20}, {20, 20, 20}, 5.0))
        {
            sample_boxes_surface(cubes_, gen, x, y, z, s, surface_density);
            i++;
        }
    ...
    cloud_frames->push_back(std::move(cubes_));
