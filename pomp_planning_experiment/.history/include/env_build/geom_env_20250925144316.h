                                  ...
                                  double r, double surface_density_pts_per_m2)
{
    ...
    for (int i = 0; i < N; ++i)
    {
        ...
        double z = cz + r * zeta;
        cloud.emplace_back(x, y, z, color);
    }
}
...
pcl::PointCloud<pcl::PointXYZ>::Ptr build_env_pointcloud(ros::NodeHandle &nh, pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud)
{

    // 3) optional knobs
    double surface_density = 30.0; // points per m^2 on surfaces
    int seed = 42;
