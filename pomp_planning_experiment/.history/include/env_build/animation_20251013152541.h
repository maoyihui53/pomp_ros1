...
private:
    ...
    std::vector<std::array<double, 3>> unit_dir_vector;
...
public:
    ...
    void next_frame()
    {
        ...
        if (idx > 0)
        {
            ...
            for (int i = 0; i < new_geom_env->get_boxes_amount(); i++)
            {
                std::array<double, 3> random_unit3D()
            }
        }
    }
};