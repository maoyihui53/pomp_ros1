...
{
    if (!param_validation<std::vector<double>>(map_size, "/param_/map/size", nh, true))
        return false;
    if (!param_validation<double>(resolution, "/param_/map/resolution", nh, false))
        return false;
    if (!param_validation<int>(balls_param.num_balls, "/param_/geometroy_env/balls/num_balls", nh, false))
        return false;
    if (!param_validation<std::vector<double>>(balls_param.radius_min_max, "/param_/geometroy_env/balls/radius", nh, true))
        return false;
    if (!param_validation<int>(cones_param.num_cones, "/param_/geometroy_env/cones/num_cones", nh, false))
        return false;
    if (!param_validation<std::vector<double>>(cones_param.radius_min_max, "/param_/geometroy_env/cones/radius", nh, true))
        return false;
    if (!param_validation<std::vector<double>>(cones_param.height_min_max, "/param_/geometroy_env/cones/height", nh, true))
        return false;

    if (!param_validation<int>(boxes_param.num_boxes, "/param_/geometroy_env/boxes/num_boxes", nh, false))
        return false;
    if (!param_validation<std::vector<double>>(arcs_param.radius_min_max, "/param_/geometroy_env/arcs/radius", nh, true))
        return false;
    if (!param_validation<std::vector<double>>(arcs_param.height_min_max, "/param_/geometroy_env/arcs/height", nh, true))
        return false;
        // std::cout<<"test_here: "<<test_here<<std::endl;
        printf(ANSI_COLOR_YELLOW "---------------------------------------------\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_GREEN "Parameters read successfully:\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_CYAN "    map_size: [%f, %f, %f]\n" ANSI_COLOR_RESET, map_size[0], map_size[1], map_size[2]);
    printf(ANSI_COLOR_CYAN "    resolution: %f\n" ANSI_COLOR_RESET, resolution);
    printf(ANSI_COLOR_CYAN "    geometroy_env:\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_CYAN "        balls:\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_CYAN "            num_balls: %d\n" ANSI_COLOR_RESET, balls_param.num_balls);
    printf(ANSI_COLOR_CYAN "            radius_min_max: [%f, %f]\n" ANSI_COLOR_RESET, balls_param.radius_min_max[0], balls_param.radius_min_max[1]);
    printf(ANSI_COLOR_CYAN "        cones:\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_CYAN "            num_cones: %d\n" ANSI_COLOR_RESET, cones_param.num_cones);
    printf(ANSI_COLOR_CYAN "            radius_min_max: [%f, %f]\n" ANSI_COLOR_RESET, cones_param.radius_min_max[0], cones_param.radius_min_max[1]);
    printf(ANSI_COLOR_CYAN "            height_min_max: [%f, %f]\n" ANSI_COLOR_RESET, cones_param.height_min_max[0], cones_param.height_min_max[1]);
    printf(ANSI_COLOR_CYAN "        arcs:\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_CYAN "            num_arcs: %d\n" ANSI_COLOR_RESET, arcs_param.num_arcs);
    printf(ANSI_COLOR_CYAN "            radius_min_max: [%f, %f]\n" ANSI_COLOR_RESET, arcs_param.radius_min_max[0], arcs_param.radius_min_max[1]);
    printf(ANSI_COLOR_CYAN "            height_min_max: [%f, %f]\n" ANSI_COLOR_RESET, arcs_param.height_min_max[0], arcs_param.height_min_max[1]);
    printf(ANSI_COLOR_YELLOW "----------------------------------------------\n" ANSI_COLOR_RESET);
    // printf(ANSI_COLOR_CYAN "    geometroy_env: balls\n" ANSI_COLOR_RESET);
    // printf(ANSI_COLOR_CYAN "        num_balls: %d\n" ANSI_COLOR_RESET, balls_param.num_balls);
