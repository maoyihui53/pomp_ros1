...
bool read_param(const ros::NodeHandle &nh)
{
    ...
    if (!param_validation<int>(balls_param.num_balls, "/param_/geometry_env/balls/num_balls", nh, false))
        return false;
    if (!param_validation<std::vector<double>>(balls_param.radius_min_max, "/param_/geometry_env/balls/radius", nh, true))
        return false;
    if (!param_validation<int>(cones_param.num_cones, "/param_/geometry_env/cones/num_cones", nh, false))
        return false;
    if (!param_validation<std::vector<double>>(cones_param.radius_min_max, "/param_/geometry_env/cones/radius", nh, true))
        return false;
    if (!param_validation<std::vector<double>>(cones_param.height_min_max, "/param_/geometry_env/cones/height", nh, true))
        return false;

    if (!param_validation<int>(arcs_param.num_arcs, "/param_/geometry_env/arcs/num_arcs", nh, false))
        return false;
    if (!param_validation<std::vector<double>>(arcs_param.radius_min_max, "/param_/geometry_env/arcs/radius", nh, true))
        return false;
    if (!param_validation<std::vector<double>>(arcs_param.height_min_max, "/param_/geometry_env/arcs/height", nh, true))
        return false;
