#include <cmath>
#include <jps_tbb/jps_planner/distance_map_planner/graph_search.h>

using namespace DMP_TBB;

GraphSearch::GraphSearch(const std::atomic<int16_t> *cMap, int xDim, int yDim, double eps,
...
GraphSearch::GraphSearch(const std::atomic<int16_t> *cMap, int xDim, int yDim, int zDim,
...
inline bool GraphSearch::isFree(int x, int y) const
{
  return x >= 0 && x < xDim_ && y >= 0 && y < yDim_ &&
         cMap_[coordToId(x, y)].load(std::memory_order_relaxed) < val_occ_;
}

inline bool GraphSearch::isFree(int x, int y, int z) const
{
  return x >= 0 && x < xDim_ && y >= 0 && y < yDim_ && z >= 0 && z < zDim_ &&
         cMap_[coordToId(x, y, z)].load(std::memory_order_relaxed) < val_occ_;
}
