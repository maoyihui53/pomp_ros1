...
template <int Dim>
DMPlanner_TBB<Dim>::DMPlanner_TBB(bool verbose) : planner_verbose_(verbose)
{
...
template <int Dim>
void DMPlanner_TBB<Dim>::setSearchRadius(const Vecf<Dim> &r)
{
  search_radius_ = r;
}

template <int Dim>
void DMPlanner_TBB<Dim>::setPotentialRadius(const Vecf<Dim> &r)
{
  potential_radius_ = r;
}

template <int Dim>
void DMPlanner_TBB<Dim>::setPotentialMapRange(const Vecf<Dim> &r)
{
  potential_map_range_ = r;
}

template <int Dim>
void DMPlanner_TBB<Dim>::setEps(double eps)
{
  eps_ = eps;
}

template <int Dim>
void DMPlanner_TBB<Dim>::setCweight(double c)
{
  cweight_ = c;
}

template <int Dim>
void DMPlanner_TBB<Dim>::setPow(int pow)
{
  pow_ = pow;
}

template <int Dim>
int DMPlanner_TBB<Dim>::status() { return status_; }

template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::getPath() { return path_; }

template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::getRawPath() { return raw_path_; }

template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::getPriorPath() { return prior_path_; }

template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::removeCornerPts(const vec_Vecf<Dim> &path)
{
  if (path.size() < 3)
    return path;
...
template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::removeLinePts(const vec_Vecf<Dim> &path)
{
  if (path.size() < 3)
    return path;
...
template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::getOpenSet() const
{
  vec_Vecf<Dim> ps;
...
template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::getCloseSet() const
{
  vec_Vecf<Dim> ps;
...
template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::getAllSet() const
{
  vec_Vecf<Dim> ps;
...
template <int Dim>
std::vector<bool> DMPlanner_TBB<Dim>::setPath(const vec_Vecf<Dim> &path,
  ...
  for (const auto &it : ps)
  {
    for (const auto &n : ns)
    {
      ...
      if (map_util_->isOutside(pn))
        continue;
      int idx = Dim == 2 ? pn(0) + dim(0) * pn(1) : pn(0) + dim(0) * pn(1) + dim(0) * dim(1) * pn(2);
      if (!in_region[idx])
      {
        in_region[idx] = true;
      }
...
template <int Dim>
vec_Vecf<Dim> DMPlanner_TBB<Dim>::getSearchRegion()
{
  auto dim = map_util_->getDim();
  vec_Vecf<Dim> pts;
...
template <int Dim>
std::shared_ptr<JPS_TBB::MapUtil<Dim>> DMPlanner_TBB<Dim>::getMapUtil()
{
  return map_util_;
}

template <int Dim>
bool DMPlanner_TBB<Dim>::checkAvailability(const Veci<Dim> &pn)
{
...
template <int Dim>
vec_Vec3f DMPlanner_TBB<Dim>::getCloud(double h_max)
{
...
template <int Dim>
vec_E<std::pair<Veci<Dim>, int16_t>> DMPlanner_TBB<Dim>::createMask(int pow)
{
  /// Mask for generating potential field around obstacle
  vec_E<std::pair<Veci<Dim>, int16_t>> mask;
  // create mask
  ...
  if (Dim == 2)
  {
    for (n(0) = -rn; n(0) <= rn; n(0)++)
    {
      for (n(1) = -rn; n(1) <= rn; n(1)++)
      {
        ...
        if (h > 1e-3)
          mask.push_back(std::make_pair(n, (int16_t)h));
      }
    }
  }
  else
...
template <int Dim>
void DMPlanner_TBB<Dim>::setMap(const std::shared_ptr<JPS_TBB::MapUtil<Dim>> &map_util,
                            const Vecf<Dim> &pos)
{
  map_util_ = std::make_shared<JPS_TBB::MapUtil<Dim>>(*map_util);
  ...
  std::atomic<int16_t> *map= map_util_->getMap();
        ...
        if (map[idx] > 0)
        {
          ...
          for (const auto &it : mask)
          {
            ...
            if (!map_util_->isOutside(new_n))
            {
              ...
              distance_map[new_idx] = std::max(distance_map[new_idx].load(std::memory_order_relaxed) , it.second);
            }
          }
        }
      }
...
template <int Dim>
bool DMPlanner_TBB<Dim>::plan(const Vecf<Dim> &start, const Vecf<Dim> &goal, decimal_t eps, decimal_t cweight)
{
  if (planner_verbose_)
  {
  ...
  if (Dim == 3)
  {
    graph_search_ = std::make_shared<DMP_TBB::GraphSearch>(
        cmap_, dim(0), dim(1), dim(2), eps, cweight, planner_verbose_);
  ...
  {
    graph_search_ = std::make_shared<DMP_TBB::GraphSearch>(
        cmap_, dim(0), dim(1), eps, cweight, planner_verbose_);
...
template <int Dim>
bool DMPlanner_TBB<Dim>::computePath(const Vecf<Dim> &start, const Vecf<Dim> &goal, const vec_Vecf<Dim> &path)
{
  if (planner_verbose_)
  {
...
// template <int Dim>
// void DMPlanner_TBB<Dim>::get_distance_path_pts()
// {
...
//   std::cout << "------------   getRawPath()  " << getPath().size() << std::endl;
//   std::cout << "------------   raw_path_.size() " << path_.size() << std::endl;
//   if (path_.size() > 1)
//   {
...
//       double step_distance = map_util_->getRes();
//       double distance_two_pts = (path_[i] - path_[i + 1]).norm();
//       double distance = 0;
//       Vecf<Dim> pt = path_[i];
//       Vecf<Dim> vector_step;
//       for (int j = 0; j < Dim; ++j)
//       {
//         vector_step[j] = std::abs(path_[i + 1][j] - path_[i][j]);
//       }
...
//       {
//         if(vector_step(j)!=0)
//             unit_step(j) = (path_[i + 1] - path_[i])[j] / vector_step(j);
//         else
//             unit_step(j) = 0;
//       }
...
//       std::cout << "distance_two_pts: " << distance_two_pts << std::endl;
//       std::cout << "step_distance: " << step_distance << std::endl;
//       std::cout << "unit_step: " << unit_step.transpose() << std::endl;

//       while (distance < distance_two_pts)
//       {
//         Veci<Dim> idx = map_util_->floatToInt(pt);
//         if (map_util_->isOutside(idx))
//           continue;


//         double clearance = static_cast<int>(cmap_[map_util_->getIndex(idx)]) * map_util_->getRes();
//         std::cout << "dist: " << static_cast<int>(cmap_[map_util_->getIndex(idx)]) << std::endl;
//         min_clearance = std::min(min_clearance, clearance);
//         pt += unit_step * step_distance;
//         distance += step_distance;
//       }

//       std::cout << "min_clearance: " << min_clearance << std::endl;
//       minDistance.push_back(min_clearance);
//     }
//   }
// }

template class DMPlanner_TBB<2>;

template class DMPlanner_TBB<3>;

template <int Dim>
IterativeDMPlanner_TBB<Dim>::IterativeDMPlanner_TBB(bool verbose) : DMPlanner_TBB<Dim>(verbose)
{
...
template <int Dim>
bool IterativeDMPlanner_TBB<Dim>::iterativeComputePath(
    ...
    const vec_Vecf<Dim> &prior_path, int max_iteration)
{
  ...
  for (int i = 0; i < max_iteration; i++)
  {
    ...
    {
      if (this->planner_verbose_)
        printf("[IterativeDMPlanner_TBB]: converged to %f!\n", prev_cost);
      break;
    }
    else
    {
      ...
      path = this->raw_path_;
      if (this->planner_verbose_)
        printf("[IterativeDMPlanner_TBB]: path cost is %f! iteration: %d\n",
               prev_cost, i);
    }
  }
...
template class IterativeDMPlanner_TBB<2>;

template class IterativeDMPlanner_TBB<3>;
