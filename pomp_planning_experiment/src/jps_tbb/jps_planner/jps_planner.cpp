...
template <int Dim>
JPSPlanner_TBB<Dim>::JPSPlanner_TBB(bool verbose): planner_verbose_(verbose) {
...
template <int Dim>
void JPSPlanner_TBB<Dim>::setMapUtil(const std::shared_ptr<JPS_TBB::MapUtil<Dim>> &map_util) {
  map_util_ = map_util;
}

template <int Dim>
int JPSPlanner_TBB<Dim>::status() { return status_; }

template <int Dim>
vec_Vecf<Dim> JPSPlanner_TBB<Dim>::getPath() { return path_; }

template <int Dim>
vec_Vecf<Dim> JPSPlanner_TBB<Dim>::getRawPath() { return raw_path_; }

template <int Dim>
vec_Vecf<Dim> JPSPlanner_TBB<Dim>::removeCornerPts(const vec_Vecf<Dim> &path) {
  if (path.size() < 2)
    return path;
...
template <int Dim>
vec_Vecf<Dim> JPSPlanner_TBB<Dim>::removeLinePts(const vec_Vecf<Dim> &path) {
  if (path.size() < 3)
    return path;
...
template <int Dim>
vec_Vecf<Dim> JPSPlanner_TBB<Dim>::getOpenSet() const {
  vec_Vecf<Dim> ps;
...
template <int Dim>
vec_Vecf<Dim> JPSPlanner_TBB<Dim>::getCloseSet() const {
  vec_Vecf<Dim> ps;
...
template <int Dim>
vec_Vecf<Dim> JPSPlanner_TBB<Dim>::getAllSet() const {
  vec_Vecf<Dim> ps;
...
template <int Dim>
void JPSPlanner_TBB<Dim>::updateMap() {
...
template <int Dim>
bool JPSPlanner_TBB<Dim>::plan(const Vecf<Dim> &start, const Vecf<Dim> &goal, decimal_t eps, bool use_jps) {
  if(planner_verbose_){
  ...
  if(Dim == 3) {
    graph_search_ = std::make_shared<JPS_TBB::GraphSearch>(cmap_.data(), dim(0), dim(1), dim(2), eps, planner_verbose_);
    graph_search_->plan(start_int(0), start_int(1), start_int(2), goal_int(0), goal_int(1), goal_int(2), use_jps);
  }
  else {
    graph_search_ = std::make_shared<JPS_TBB::GraphSearch>(cmap_.data(), dim(0), dim(1), eps, planner_verbose_);
...
template class JPSPlanner_TBB<2>;

template class JPSPlanner_TBB<3>;
