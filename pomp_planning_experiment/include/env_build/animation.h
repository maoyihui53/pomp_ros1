...
class geom
{
    // Store points as columns: 3 x N
    Eigen::Matrix3Xd geom_pos;
    int idx;
...
public:
    ...
    {
        geom_pos.col(idx) = pos;  // column-wise insert
        idx++;
    }

    Eigen::Matrix3Xd get_geom_pos() { return geom_pos; }

    int amount() { return idx; }

    void move_step(const std::array<double, 3> &step, int dir)
    {
        (void)dir; // dir not needed with explicit delta; keep to preserve API
        Eigen::Vector3d stp(step[0], step[1], step[2]);
        geom_pos.colwise() += stp;
    }
};
...
public:
    ...
    void set_centers(const Eigen::Vector3d &center) { centers.emplace_back(center); }
    void set_half_size(double hs) { half_sizes.emplace_back(hs); }

    void set_dir() { dir.resize(boxes_param.num_boxes, 1); }

    void push_back(const geom &cube) { cloud_boxes.emplace_back(cube); }
    ...
    {
        int idx_ = 0;
        for (auto &b : cloud_boxes) idx_ += b.amount();
        idx = idx_;
        return idx;
    }

    std::vector<geom> get_boxes() { return cloud_boxes; }
    ...
    int get_boxes_amount() { return static_cast<int>(cloud_boxes.size()); }
    ...
    void move_one_step()
    {
        ...
        for (int i = 0; i < static_cast<int>(cloud_boxes.size()); ++i)
        {
            ...
            if (dir_vec.norm() > 1e-12) dir_vec.normalize();
            else                        dir_vec = Eigen::Vector3d(1.0, 0.0, 0.0);
...
public:
    ...
    {
        frames.emplace_back(std::move(g));
    }

    void print_info(int idx = 0) { frames[idx]->print_info(); }
    ...
    int pd_amount_in_frame(int i) { return frames[i]->pd_amount(); }
    ...
    // Concatenate point clouds horizontally: 3 x (N_old + N_chunk)
    void append(Eigen::Matrix3Xd &pd, const Eigen::Matrix3Xd &chunk)
    {
        if (chunk.cols() == 0) return;

        const auto old = pd.cols();
        pd.conservativeResize(Eigen::NoChange, old + chunk.cols());
        pd.block(0, old, 3, chunk.cols()) = chunk;
    }

    Eigen::Matrix3Xd get_pd_in_frame(int idx)
    {
        int pd_amount = pd_amount_in_frame(idx);
        std::cout << "pd amount: " << pd_amount << std::endl;
        ...
        {
            Eigen::Matrix3Xd pd_box = frames[idx]->get_boxes()[i].get_geom_pos();
            append(pd, pd_box);
        }
    ...
    void next_frame()
    {
        int idx = static_cast<int>(frames.size()); // next_idx
        if (idx > 0)
        {
...
//     void append(Eigen::MatrixX3d &pd, const Eigen::MatrixX3d &chunk)
//     {
//         if (chunk.rows() == 0)
//             return;

//         const auto old = pd.rows();
//         pd.conservativeResize(old + chunk.rows(), Eigen::NoChange);
//         pd.block(old, 0, chunk.rows(), 3) = chunk;
//     }
