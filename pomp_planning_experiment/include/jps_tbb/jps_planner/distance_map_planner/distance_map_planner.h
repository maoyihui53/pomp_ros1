...
#include <jps_tbb/jps_collision/map_util.h>
...
public:
   ...
   */
  DMPlanner_TBB(bool verbose = false);
  ...
  std::shared_ptr<JPS_TBB::MapUtil<Dim>> getMapUtil();
   ...
   * the internal map.
   */
  void setMap(const std::shared_ptr<JPS_TBB::MapUtil<Dim>> &map_util,
              const Vecf<Dim> &pos);

  /// Compute the optimal path
  bool computePath(const Vecf<Dim> &start, const Vecf<Dim> &goal, const vec_Vecf<Dim> &path);

  //  void get_distance_path_pts();
protected:
  /// Create the mask for potential distance field
  vec_E<std::pair<Veci<Dim>, int16_t>> createMask(int pow);
  ...
  std::shared_ptr<JPS_TBB::MapUtil<Dim>> map_util_;
  /// The planner back-end
  std::shared_ptr<DMP_TBB::GraphSearch> graph_search_;
  /// tunnel for visualization
  std::vector<bool> search_region_;
  /// 1-D map array
  // std::vector<int8_t> cmap_;
  std::atomic<int16_t> *cmap_;
...
typedef DMPlanner_TBB<2> DMPlanner_TBB2D;

/// Planner for 3D VoxelMap
typedef DMPlanner_TBB<3> DMPlanner_TBB3D;

template <int Dim>
class IterativeDMPlanner_TBB : public DMPlanner_TBB<Dim>
{
public:
  IterativeDMPlanner_TBB(bool verbose = false);
...
typedef IterativeDMPlanner_TBB<2> IterativeDMPlanner_TBB2D;

/// Iterative Planner for 3D VoxelMap
typedef IterativeDMPlanner_TBB<3> IterativeDMPlanner_TBB3D;
