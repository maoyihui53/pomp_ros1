/**
 * @file jps_planner.h
 * @brief JPSPlanner_TBB
 */
#pragma once

#include <jps_basis/data_type.h>
#include <jps_tbb/jps_planner/jps_planner/graph_search.h>
...
class JPSPlanner_TBB
{
  public:
     ...
     */
    JPSPlanner_TBB(bool verbose = false);

    ///Set map util for collistion checking
    void setMapUtil(const std::shared_ptr<JPS_TBB::MapUtil<Dim>>& map_util);
    /**
  ...
  protected:
    ...
    ///The planner
    std::shared_ptr<JPS_TBB::GraphSearch> graph_search_;
...
typedef JPSPlanner_TBB<2> JPSPlanner_TBB2D;

///Planner for 3D VoxelMap
typedef JPSPlanner_TBB<3> JPSPlanner_TBB3D;
