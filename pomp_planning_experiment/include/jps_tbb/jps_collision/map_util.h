...
namespace JPS_TBB
{
  ...
  public:
    ...
    bool isFree(int idx) { return map_[idx].load(std::memory_order_relaxed) == val_free; }
    /// Check if the cell is unknown by index
    bool isUnknown(int idx) { return map_[idx].load(std::memory_order_relaxed) == val_unknown; }
    /// Check if the cell is occupied by index
    bool isOccupied(int idx) { return map_[idx].load(std::memory_order_relaxed) > val_free; }
    ...
    bool isUnknown(const Veci<Dim> &pn)
    {
      if (isOutside(pn))
        return false;
      return map_[getIndex(pn)].load(std::memory_order_relaxed) == val_unknown;
    }
     ...
     */
    void setMap(const Vecf<Dim> &ori, const Veci<Dim> &dim, Tmap *map, decimal_t res)
    {
      map_ = map;
      dim_ = dim;
      origin_d_ = ori;
    ...
    bool isBlocked(const Vecf<Dim> &p1, const Vecf<Dim> &p2, int16_t val = 100)
    {
      vec_Veci<Dim> pns = rayTrace(p1, p2);
      for (const auto &pn : pns)
      {
        if (map_[getIndex(pn)].load(std::memory_order_relaxed) >= val)
          return true;
      }
      return false;
    }
      ...
      if (Dim == 3)
      {
        for (n(0) = 0; n(0) < dim_(0); n(0)++)
        {
          for (n(1) = 0; n(1) < dim_(1); n(1)++)
          {
            ...
            {
              if (isUnknown(getIndex(n)))
                map_[getIndex(n)].load(std::memory_order_relaxed) = val_free;
            }
          }
        }
      }
    ...
    // std::atomic<int16_t> *map_atomic_ = nullptr;
